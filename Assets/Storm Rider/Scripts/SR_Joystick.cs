...
public class SR_Joystick : MonoBehaviour
{
    ...
    */
	...
	private static List<SR_Joystick> joysticks;
	...
	private void Awake()
	{
		...
		if (!enumeratedJoysticks)
		{
			try
			{
				...
				joysticks = new List<SR_Joystick>(objs.Length);
				foreach (GameObject obj in objs)
				{
					SR_Joystick newJoystick = obj.GetComponent<SR_Joystick>();
					if (newJoystick == null)
					{
		...
		else
		{
			for (int i = 0; i < count; i++)
			{
				...
				if (shouldLatchFinger && (lastFingerId == -1 || lastFingerId != touch.fingerId))
				{
					...
					foreach (SR_Joystick j in joysticks)
					{
						if (j == this)
						{
							continue;
